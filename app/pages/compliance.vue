...
// Navigation State
// ──────────────────────────────────────────────
const activeSection = ref("info"); // 'info', 'registration', 'documents'

const sections = [
  { id: "info", label: "Business Info", icon: Building2, desc: "General details" },
  { id: "registration", label: "Registration", icon: ShieldCheck, desc: "Corporate & Tax" },
  { id: "documents", label: "Documents", icon: FolderOpen, desc: "Compliance files" },
];
...
<template>
  <div class="min-h-screen bg-muted/30 text-foreground pb-12">
    ...
    <header class="border-b bg-background sticky top-0 z-20">
      <div class="container max-w-6xl mx-auto flex items-center justify-between py-4 px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-4">
          <Button variant="ghost" size="icon" @click="navigateBack" class="h-9 w-9 rounded-full bg-muted/50 hover:bg-muted">
            <ArrowLeft class="w-4 h-4" />
          </Button>
          <div>
            <h1 class="text-xl font-bold tracking-tight">Compliance & Settings</h1>
        ...
        <div class="hidden sm:flex items-center gap-3">
          <div class="text-sm text-right">
            ...
            <p class="text-xs text-muted-foreground">{{ completionPercentage }}% Complete</p>
          </div>
          <div class="w-24 h-2 bg-muted rounded-full overflow-hidden">
            <div class="h-full bg-primary transition-all duration-500 ease-in-out" :style="{ width: `${completionPercentage}%` }"></div>
          </div>
        </div>
      </div>
    </header>
      ...
      <div class="flex flex-col md:flex-row gap-8">
        ...
        <aside class="md:w-64 shrink-0">
          <div class="sticky top-28 space-y-1">
            ...
            <div class="flex overflow-x-auto md:flex-col gap-2 pb-4 md:pb-0 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
              <button
                ...
                :class="activeSection === section.id ? 'bg-primary text-primary-foreground shadow-md' : 'hover:bg-muted text-muted-foreground'"
              >
                <component :is="section.icon" class="w-5 h-5" :class="activeSection === section.id ? 'text-primary-foreground' : 'text-foreground'" />
                <div>
                  <p class="font-medium text-sm" :class="activeSection === section.id ? 'text-primary-foreground' : 'text-foreground'">{{ section.label }}</p>
                  <p class="text-xs hidden md:block opacity-80" :class="activeSection === section.id ? 'text-primary-foreground/80' : 'text-muted-foreground'">{{ section.desc }}</p>
                </div>
              </button>
            </div>
            ...
            <div class="mt-6 p-4 rounded-xl bg-card border shadow-sm md:hidden">
              <div class="flex justify-between items-center mb-2">
                ...
                <p class="text-sm font-bold text-primary">{{ completionPercentage }}%</p>
              </div>
        ...
        <div class="flex-1 space-y-6 max-w-3xl">
          ...
          <div v-if="activeSection === 'info'" class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div>
              <h2 class="text-2xl font-bold tracking-tight">Business Information</h2>
              <p class="text-muted-foreground mt-1">Update your general business details and public profile.</p>
            </div>
              ...
              <CardContent class="p-6 space-y-6">
                <div class="grid gap-6">
                  ...
                  <div class="space-y-2">
                    ...
                    <Textarea v-model="infoData.about" placeholder="Describe your business..." class="min-h-[120px] resize-y" />
                    <p class="text-[11px] text-muted-foreground">Briefly explain what your business does and the services you provide.</p>
                  </div>
                </div>
          ...
          <div v-if="activeSection === 'registration'" class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div>
              <h2 class="text-2xl font-bold tracking-tight">Registration Details</h2>
              <p class="text-muted-foreground mt-1">Manage your corporate registration numbers and tax identification.</p>
            </div>

            <Card class="border shadow-sm">
              <CardContent class="p-6 space-y-6">
                
                <div class="bg-primary/5 border border-primary/20 rounded-lg p-4 flex gap-3">
                  <AlertCircle class="w-5 h-5 text-primary shrink-0 mt-0.5" />
                  <div>
                    <h4 class="text-sm font-semibold text-foreground">Why do we need this?</h4>
                    <p class="text-xs text-muted-foreground mt-1 leading-relaxed">
                      Your registration and tax details are required to verify your business entity for regulatory compliance. This information is kept secure.
                    </p>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                  <div class="space-y-2">
                    <Label class="text-sm font-semibold">Registration Number (RC)</Label>
                    <Input v-model="regData.number" placeholder="e.g. RC-123456" />
                    <p class="text-[11px] text-muted-foreground">Your corporate affairs commission number.</p>
                  </div>
                  <div class="space-y-2">
                  ...
                  <div class="space-y-2">
                    <Label class="text-sm font-semibold">Country of Incorporation</Label>
                    <Input v-model="regData.country_of_incorporation" placeholder="e.g. Nigeria" />
                  </div>
                  <div class="space-y-2">
                    <Label class="text-sm font-semibold">Date of Incorporation</Label>
          ...
          <!-- Documents Section -->
          <div v-if="activeSection === 'documents'" class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div>
              <h2 class="text-2xl font-bold tracking-tight">Compliance Documents</h2>
              <p class="text-muted-foreground mt-1">Upload and manage necessary verification files.</p>
            </div>

            <!-- Upload New Document Form -->
            <Card class="border-2 border-dashed border-primary/20 shadow-none bg-primary/5">
              ...
              <CardContent class="space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div class="space-y-2">
                    <Label class="text-sm font-semibold">Document Type <span class="text-destructive">*</span></Label>
                      ...
                      <SelectContent>
                        ...
                        >
                          <div class="flex items-center justify-between w-full pr-2">
                            <span>{{ doc.name }}</span>
                            <Badge v-if="doc.required" variant="secondary" class="text-[9px] h-4 px-1 ml-2">Req</Badge>
                          </div>
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  ...
                  <div class="space-y-2">
                    ...
                    <Input v-model="uploadData.description" placeholder="Brief note about the file" class="bg-background" />
                  </div>
                </div>

                <div class="space-y-2">
                  <Label class="text-sm font-semibold">File <span class="text-destructive">*</span></Label>
                  <div 
                  ...
                  >
                    ...
                    <p class="font-medium text-sm mb-1">
                      {{ uploadData.fileObj ? uploadData.fileObj.name : 'Click to select a file' }}
                    </p>
                    <p class="text-xs text-muted-foreground">
                      {{ uploadData.fileObj ? `${(uploadData.fileObj.size / 1024 / 1024).toFixed(2)} MB` : 'PDF, JPG, or PNG up to 5MB' }}
                    </p>
                    <Input 
                      type="file" 
                      ref="fileInput"
            ...
            <div class="pt-4">
              ...
              <div v-else-if="!businessDocuments.data.value?.getDocuments?.length" class="py-16 text-center border rounded-xl bg-card shadow-sm">
                <div class="w-16 h-16 rounded-full bg-muted flex items-center justify-center mx-auto mb-4">
                  <FolderOpen class="w-8 h-8 text-muted-foreground/60" />
                </div>
                <h3 class="text-lg font-semibold text-foreground">No documents uploaded</h3>
                <p class="text-sm text-muted-foreground max-w-xs mx-auto mt-1">Files you upload will appear here securely for verification.</p>
              </div>
              ...
              <div v-else class="grid gap-3">
                ...
                >
                  <div class="flex items-center gap-4 overflow-hidden">
                    ...
                    <div class="truncate">
                      <p class="font-semibold text-sm text-foreground truncate">{{ doc.description || doc.document_id }}</p>
                      <a :href="doc.url" target="_blank" class="text-[11px] font-medium text-primary hover:underline mt-0.5 inline-flex items-center gap-1">
                        View Secure File ↗
                      </a>
                    </div>
                  </div>
