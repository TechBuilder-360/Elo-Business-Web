import { defineEventHandler, readBody, getHeaders, createError, getCookie, setCookie } from "h3";
...
export default defineEventHandler(async (event) => {
  ...
  const headers: Record<string, string> = {
    ...
    ...(rawHeaders["user-agent"] ? { "User-Agent": rawHeaders["user-agent"] } : {}),
    ...(rawHeaders["accept"] ? { Accept: rawHeaders["accept"] } : {}),
    ...(rawHeaders.authorization ? { Authorization: rawHeaders.authorization } : {}),
  };
