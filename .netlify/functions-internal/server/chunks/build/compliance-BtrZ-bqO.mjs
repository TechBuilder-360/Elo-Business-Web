import { computed, ref, watch, mergeProps, unref, withCtx, createVNode, openBlock, createBlock, resolveDynamicComponent, createTextVNode, toDisplayString, createCommentVNode, Fragment, renderList, useSSRContext } from 'vue';
...
import { f as useRoute, h as useRouter } from './server.mjs';
...
import { ArrowLeft, Moon, Sun, Building2, FolderOpen, Pencil, ShieldCheck, AlertCircle, Loader2, Save, UploadCloud, FileText, CheckCircle2, Trash2 } from 'lucide-vue-next';
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const { data: fullBusinessDataResult, isPending: isDetailPending } = getBusinessDetailQuery(computedActiveId);
    ...
    const infoData = ref({
      name: "",
      // will be populated from the backend — never use route.query.name here
      about: "",
      industry: "",
      website: ""
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-muted/30 text-foreground pb-12" }, _attrs))}><header class="border-b bg-background sticky top-0 z-20"><div class="container max-w-8xl mx-auto flex items-center justify-between py-4 px-4 sm:px-6 lg:px-8"><div class="flex items-center gap-4">`);
      ...
      _push(`<div><h1 class="text-xl font-bold tracking-tight"> Compliance &amp; Settings </h1><p class="text-sm text-muted-foreground flex items-center gap-2">${ssrInterpolate(businessName.value)}</p></div></div><div class="hidden sm:flex items-center gap-3">`);
      _push(ssrRenderComponent(unref(Button), {
        variant: "ghost",
        size: "icon",
...
//# sourceMappingURL=compliance-BtrZ-bqO.mjs.map
