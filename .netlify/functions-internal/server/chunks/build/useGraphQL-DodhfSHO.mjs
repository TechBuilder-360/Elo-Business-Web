...
async function gqlRequest({ query, variables = {} }) {
  ...
  if (response.errors?.length) {
    response.errors.some(
      (e) => e.message?.toLowerCase().includes("unauthenticated") || e.message?.toLowerCase().includes("unauthorized") || e.message?.toLowerCase().includes("unauthorised") || e.extensions?.code === "UNAUTHENTICATED" || e.extensions?.code === "FORBIDDEN"
    );
...
function useGQLQuery(key, query, variables = {}, opts = {}) {
  return useQuery({
    queryKey: key,
    queryFn: () => {
      const resolvedVars = typeof variables === "function" ? variables() : variables && typeof variables.value !== "undefined" ? variables.value : variables;
...
//# sourceMappingURL=useGraphQL-DodhfSHO.mjs.map
